...
const DEFAULT_SNACKBAR_TIMEOUT = 10_000;
...
export async function showForRange(range: Range, exactMatch: boolean = false) {
  ...
  // prevent activation inside of application
  if (!isForSite() && window.location.origin === config.URIFront) {
    return;
  }

  if (!state.user) {
    showSnackbar(t('please_login'), { duration: DEFAULT_SNACKBAR_TIMEOUT });
    return;
  }
