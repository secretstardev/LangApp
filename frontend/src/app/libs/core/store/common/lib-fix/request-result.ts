// https://github.com/ngneat/elf/blob/6fe69804451bebf7cffff798c59b036303399258/packages/requests/src/lib/requests-result.ts#L163

import { BehaviorSubject, combineLatest, EMPTY, filter, map, MonoTypeOperatorFunction, Observable, of, OperatorFunction, pipe, switchMap, take, tap, throwError } from 'rxjs';
...
export type RequestResult<TError = any, TData = any> = LoadingRequestResult | SuccessRequestResult<TData> | ErrorRequestResult<TError> | IdleRequestResult;
...
function getSource<TValue>(key: unknown[], initialValue: TValue, map: Map<string, BehaviorSubject<TValue>>) {
...
// @public
export function getRequestResult<TError>(key: unknown[], { initialStatus }: { initialStatus?: 'idle' } = {}): Observable<RequestResult<TError>> {
  ...
  return getSource<RequestResult>(key, result, emitters) as Observable<RequestResult<TError>>;
}
...
function updateRequestResult(key: unknown[], newValue: Partial<RequestResult>) {
  ...
  if (result) {
    ...
    for (const key of Object.keys(newValue)) {
      if (currentResult[key as keyof RequestResult] !== newValue[key as keyof RequestResult]) {
        hasChange = true;
        break;
      }
    }
...
// @public
export function joinRequestResult<T, TError = any, TData = any>(...[key, options]: Parameters<typeof getRequestResult>): OperatorFunction<T, RequestResult<TError, TData> & { data: T }> {
...
export function trackRequestResult<TData>(key: unknown[], options?: Options<TData>): MonoTypeOperatorFunction<TData> {
...
export function filterSuccess<TData>(): OperatorFunction<RequestResult & { data: TData }, SuccessRequestResult & { data: TData }> {
  return filter((result): result is SuccessRequestResult & { data: TData } => result.status === 'success');
}

export function filterError<TError>(): OperatorFunction<RequestResult<TError>, ErrorRequestResult<TError>> {
  return filter((result): result is ErrorRequestResult<TError> => result.status === 'error');
}

export function mapResultData<TData, R>(mapFn: (data: NonNullable<TData>) => R): MonoTypeOperatorFunction<RequestResult & { data: TData }> {
  return pipe(
    map((result) => {
      return {
        ...result,
