...
import { In, LessThanOrEqual, Repository, Raw, MoreThan, Brackets } from 'typeorm';
...
import { DrillSettings, UpdateDrillSettingsDto } from './dto/drill.settings.dto';
...
export class DrillsGenerator {
  ...
  async getList(listId?: number, moreNewWords?: number) {
    const optionsLoadWords: LoadWordsOptions = {
      listId,
      moreNewWords: moreNewWords ?? 0,
    };

    await this.loadWords(optionsLoadWords);
    ...
    for (const userWord of this.userWords) {
      ...
      const currentExtractedKanji = extractKanji(currentWord.data?.readings?.[0]?.kanji);
      const currentUserKanjis = currentExtractedKanji.map((extractedKanji) => this.userKanjis.find((k) => k.original_word == extractedKanji));
      if (uniq(currentExtractedKanji).length != currentUserKanjis.filter((i) => i).length) {
      ...
      for (const [i, userKanji] of currentUserKanjis.entries()) {
        ...
        if (!currentKanji) {
          console.log(`Skip user kanji ${userKanji?.id} (${currentExtractedKanji[i]}): no kanji in dictionary with such id`);
          continue;
        }
    ...
    this.drills = await this.filterDrillsBySettings(this.drills);

    return {
      cards: this.cards,
      drills: this.drills,
      userWords: this.isTestMode ? this.userWords : undefined,
      words: this.isTestMode ? this.words : undefined,
      kanjis: this.isTestMode ? this.kanjis : undefined,
    };
  }

  async applyHideSettings(drills: Drill[], mode?: string, cardToHide?: string) {
    const drillSettings = this.getSettings();
    if (mode === 'disableAudioQuestionsFor1Hour') {
      drillSettings.disableAudioQuestionsUntil = new Date(Date.now() + 60 * 60 * 1000).toISOString();
    }

    if (cardToHide) {
      const drill = drills.find((drill) => drill.card === cardToHide);

      if (drill) {
        if (mode === 'onlyCurrentQuestion') {
          if (!drillSettings.disabledCardIds.includes(drill.card)) {
            drillSettings.disabledCardIds.push(drill.card);
          }
        } else if (mode === 'allWithCurrentWord') {
          if (!drillSettings.disabledWordIds.includes(drill.trackBy)) {
            drillSettings.disabledWordIds.push(drill.trackBy);
          }

          await this.userDictionaryRepository.update({ dictionary_word_id: Number(cardToHide), user_id: this.user.id }, { drill_due: new Date('3000-01-01T00:00:00Z') });
        } else if (mode === 'disableCardType') {
          if (!drillSettings.disabledCardTypes.includes(drill.cardType)) {
            drillSettings.disabledCardTypes.push(drill.cardType);
          }
        }
      }
    }

    this.updateSettings(drillSettings);
  }

  async filterDrillsBySettings(drills: Drill[]) {
    const drillSettings = this.getSettings();
    const now = new Date();
    let { disabledCardTypes = [], disabledCardIds = [], disabledWordIds = [], disableAudioQuestionsUntil } = drillSettings;

    drills = drills.map((drill) => {
      const cardType = drill.cardType;

      const isDisabledCardType = disabledCardTypes.includes(cardType);
      const isDisabledCardId = disabledCardIds.includes(drill.card);
      const isDisabledWordId = disabledWordIds.includes(drill.trackBy);
      const isDisabledAudioQuestion = disableAudioQuestionsUntil && new Date(disableAudioQuestionsUntil) >= now && this.isAudioQuestion(drill.cardType);

      const hide = isDisabledCardType || isDisabledCardId || isDisabledWordId || isDisabledAudioQuestion || false;
  ...
  private isAudioQuestion(cardType: string): boolean {
    return cardType == 'selectWordForAudio' || cardType == 'selectAudioForWord';
  }

  private async loadKanji() {
    // TODO: add "usedKanji" to this.words dictionary (and/or user dictionary)

    const extractedKanji = this.words.flatMap((w) => extractKanji(w.data?.readings?.[0]?.kanji));
  ...
  private async getCountNewWordsToLoad(options: LoadWordsOptions = {}) {
    const userToday = moment().tz(this.user.timezone).startOf('day').toDate();

    const countNewWordsStudiedToday = await this.userDictionaryRepository
      .createQueryBuilder('dictionary')
      .where('dictionary.user_id = :userId', { userId: this.user.id })
      .andWhere('dictionary.type = :type', { type: DictionaryType.JapaneseWords })
      .andWhere(
        new Brackets((qb) => {
          qb.where("dictionary.drill_card->>'countAnswers' = :one", { one: '1' });
        }),
      )
      .andWhere(options.listId ? 'dictionary.listIds @> ARRAY[:listId]::integer[]' : 'TRUE', { listId: options.listId })
      .andWhere('dictionary.drill_last > :userToday', { userToday })
      .getCount();

    const newWordsDailyLimit = this.user.dataJson.newWordsDailyLimit || 30;

    return Math.max(options.moreNewWords, newWordsDailyLimit - countNewWordsStudiedToday);
  }

  private async loadAdditionalNewWords(wordsLength: number, options: LoadWordsOptions = {}) {
    const countNewWordsToLoad = await this.getCountNewWordsToLoad(options);

    const maxNewWords = 10 - wordsLength;
    const countNewWords = Math.min(countNewWordsToLoad, maxNewWords);

    const additionalNewDictionary = await this.userDictionaryRepository
      .createQueryBuilder('dictionary')
      .where('dictionary.user_id = :userId', { userId: this.user.id })
      .andWhere('dictionary.type = :type', { type: DictionaryType.JapaneseWords })
      .andWhere(
        new Brackets((qb) => {
          qb.where("dictionary.drill_card->>'countAnswers' = :zero", { zero: '0' }).orWhere("dictionary.drill_card->>'countAnswers' IS NULL");
        }),
      )
      .andWhere(options.listId ? 'dictionary.listIds @> ARRAY[:listId]::integer[]' : 'TRUE', { listId: options.listId })
      .orderBy('dictionary.drill_due', 'ASC')
      .take(countNewWords)
      .getMany();

    this.userWords = [...this.userWords, ...additionalNewDictionary];
  }

  private async loadWords(options: LoadWordsOptions = {}) {
    this.userWords = await this.userDictionaryRepository
      .createQueryBuilder('dictionary')
      .where('dictionary.user_id = :userId', { userId: this.user.id })
      .andWhere('dictionary.type = :type', { type: DictionaryType.JapaneseWords })
      .andWhere('dictionary.drill_due <= :currentDate', { currentDate: new Date() })
      .andWhere(
        new Brackets((qb) => {
          qb.where("(dictionary.drill_card->>'countAnswers')::int > 1").andWhere("(dictionary.drill_card->>'countAnswers') IS NOT NULL");
        }),
      )
      .andWhere(options.listId ? 'dictionary.listIds @> ARRAY[:listId]::integer[]' : 'TRUE', { listId: options.listId })
      .orderBy('dictionary.drill_due', 'ASC')
    ...
    if (this.userWords.length < 10) {
      await this.loadAdditionalNewWords(this.userWords.length, options);
    }
  ...
  *getSimilarRandomWords(word: JapaneseWord) {
  ...
  getSettings() {
    return plainToInstance(DrillSettings, this.user.dataJson.drillSettings || {}, { exposeDefaultValues: true });
  }

  async updateSettings(drillSettings: UpdateDrillSettingsDto) {
    const currentDrillSettings = this.getSettings();

    const updatedDrillSettings = {
      ...currentDrillSettings,
      ...drillSettings,
    };
    this.user.dataJson.drillSettings = updatedDrillSettings;

    await this.userRepository
      .createQueryBuilder()
      .update(User)
      .set({ dataJson: () => `jsonb_set("dataJson", '{drillSettings}', '${JSON.stringify(updatedDrillSettings)}')` })
