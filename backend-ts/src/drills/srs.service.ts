...
export class SrsService {
  ...
  async processAnswer(userWord: UserDictionary, answer: number) {
    ...
    review.userId = userWord.user_id;
    ...
    userWord.drill_progress = this.calculateProgress(card);
  ...
  protected calculateProgress(card: UserDictionaryDrillCard) {
    const learnedInterval = 21; // Interval after which the card is considered learned
    // If the interval is 21 days, then progress is 100%.
    // If the interval is 7 days, then progress is 67%.
    // If the interval is 3 days, then progress is 45%.
    const percentFraction = Math.log2(1 + card.interval) / Math.log2(1 + learnedInterval);
    const progress = Math.min(100, Math.round(percentFraction * 100));
    return progress;
  }
