...
import { DrillsGenerator } from './drills/drills.generator';

@Injectable()
export class AppService {
  private readonly logger = new Logger(AppService.name, { timestamp: true });

  constructor(private dictionaryWordRepository: DictionaryWordRepository, private jumanppClient: JumanppClient, private analyzeJapanese: AnalyzeJapaneseService, private drillsGenerator: DrillsGenerator) {}
  ...
  async processText(originalText: string, clickedOffsetOriginal: number, requestedLanguages: string[], exactMatch: boolean = false): Promise<any> {
    ...
    for (let word of wordsFromDb) {
      ...
      if (variant) {
        let audioUrls = this.drillsGenerator.getAudioUrlsForWord(word);
        resultWords.push({
          id: word.id,
          type: word.type,
          ...word.data,
