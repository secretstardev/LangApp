...
export class AudioController {
  ...
  async getItem(@Req() req: RequestWithUser, @Query() query, @Res() res: any) {
    ...
    if (!this.speechServices.amazon || !this.speechServices.azure) {
      res.redirect(302, 'https://langapp-master.metal1.getinfura.com/api/audio?q=' + encodeURIComponent(query?.q));
      return;
    }
