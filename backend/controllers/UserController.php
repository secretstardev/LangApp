...
use app\models\UserDictionaryQuery;
...
class UserController extends ActiveController
{
    ...
    public function behaviors()
    {
        ...
        $behaviors['access'] = [
            ...
            'rules' => [
                ...
                [
                    'allow' => true,
                    'actions' => ['me', 'update', 'invited-users', 'close-notification', 'my-payment-methods', 'add-card-square', 'delete-payment-method', 'prolong-subscription', 'stripe-setup-intent', 'stripe-add-payment-method', 'stats'],
                    'roles' => ['@'],
                ],
                [
                    'allow' => true,
    ...
    public function actionCreate()
    {
        ...
        if (empty($model->country)) {
            $model->country = Helpers::getCountryCodeFromRequestHeader();
        }

        $model->paidUntilDateTime = '2000-01-01 00:00:00'; // default value for this field, used to detect "never subscribed" in frontend
        if ($model->save()) {
    ...
    public function actionStripeAddPaymentMethod()
    {
        ...
        if ($customerId != null) {
            ...
            if ($paymentMethod->save(false)) {
                $user->paySubscriptionIfNecessary();
            }
    ...
    public function actionStats()
    {
        ...
        $totalActivitySeconds = ActivityStats::find()->user($userId)->sum('[[total_seconds]]');
        $totalInterimSeconds = ActivityInterim::find()->user($userId)->sum('[[seconds]]');
        $totalSecondsStudied = $totalActivitySeconds + $totalInterimSeconds;

        $countVideoStudied = ContentAttribute::find()->user($userId)
            ->andWhere(['[[isStudied]]' => 1])
            ->count();

        $countWordsAdded = UserDictionary::find()->user($userId)->count();
        $countDrillsDone = DrillReview::find()->user($userId)->count();

        return [
            'totalSecondsStudied' => $totalSecondsStudied,
            'countVideoStudied' => $countVideoStudied,
            'countWordsAdded' => $countWordsAdded,
            'countDrillsDone' => $countDrillsDone,
        ];
    }
