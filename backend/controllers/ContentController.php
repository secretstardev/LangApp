...
class ContentController extends ActiveController
{
    ...
    {
        $model = Content::find()->with('categories')->where(['id' => $id])->one();
        if ($model === null) {
    ...
    /**
     * Return a random video by user language level and one of their interest
     * @return Content
     */
    public function actionRandom()
    {
        ...
        $userLevel = empty($user->languageLevel) ? LanguageLevel::NEW : $user->languageLevel;
        $level = LanguageLevel::getInteger($userLevel);
        if ($level > 0) {
            $query->andWhere(['[[t]].[[level]]' => $level]);
        }

        $favoriteCategoryIds = is_array($user->favoriteCategoryId) ? array_values($user->favoriteCategoryId) : [];
        if (!empty($favoriteCategoryId)) {
            $query->andWhere(['{{content_category}}.[[category_id]]' => $favoriteCategoryIds]);
        }

        $randomSeed = mt_rand() / mt_getrandmax();
        $query->orderBy(new Expression('SETSEED(:order_seed * 1.0 / "t"."id") || \'\', RANDOM()', [':order_seed' => $randomSeed]));
