...
class Transaction extends \yii\db\ActiveRecord
{
    ...
    public function getMoneyInSmallestUnit()
    {
        ...
        // From https://stripe.com/docs/currencies?presentment-currency=JP#three-decimal
        $threeDecimalCurrencies = ['BHD', 'JOD', 'KWD', 'OMR', 'TND'];
        if (in_array($currency, $threeDecimalCurrencies)) {
            // last digit must be zero
            return (int) (round($this->money, 2) * 1000);
        }

        return (int) round($this->money * 100);
    }
    ...
    public function fields()
    {
        ...
        if (Helpers::isAdmin() && (!static::isSaveMethod())) {
            ...
            $fields = ['id', 'userId', 'money', 'comment', 'addedDateTime' => [Helpers::class, 'formatDateField'], 'isPartner', 'fromInvitedUserId'];
        }

        $fields['status'] = function ($model) {
            return TransactionStatus::getString($model->status);
        };
