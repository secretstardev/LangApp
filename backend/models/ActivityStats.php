...
class ActivityStats extends ActiveRecord
{
    ...
    public static function factory($user, $date)
    {
        ...
        $sinceUserCreated = (new DateTimeImmutable($user->addedDateTime))->diff(new DateTimeImmutable());
        $enoughTimeAfterRegistration = $sinceUserCreated->days > 1 || $sinceUserCreated->h >= 4;
        if ($user->tariff == UserTariff::FREE && $enoughTimeAfterRegistration && $user->penaltyAmount > 0 && $model->is_goal_reached === false) {
            $model->is_penalty_received = true;
        }
