...
class PaymentMethod extends \yii\db\ActiveRecord
{
    ...
    {
        $data = empty($this->data) || !is_array($this->data) ? [] : $this->data;
        if ($this->type === static::TYPE_STRIPE) {
            return (int) ArrayHelper::getValue($data, ['card', 'exp_year']);
        } elseif ($this->type === static::TYPE_SQUARE) {
            return (int) ArrayHelper::getValue($data, ['expYear']);
        }
        return null;
    }
    ...
    public function getExpMonth()
    {
        $data = empty($this->data) || !is_array($this->data) ? [] : $this->data;
        if ($this->type === static::TYPE_STRIPE) {
            return (int) ArrayHelper::getValue($data, ['card', 'exp_month']);
        } elseif ($this->type === static::TYPE_SQUARE) {
            return (int) ArrayHelper::getValue($data, ['expMonth']);
        }
        return null;
    }
    ...
    public function getSubtitle()
    {
        ...
        if (!empty($year) && !empty($month)) {
            return sprintf('%02s / %s', $month, $year);
        }
    ...
    {
        if ($amount == 0) {
            throw new BadRequestHttpException('Amount is not allowed: ' . $amount);
        }
